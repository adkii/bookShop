...
public final class Strings {
    ...
    public static  BigDecimal judge(String value){
        return Strings.isNullOrEmpty(value)? null: new BigDecimal(value).setScale(3, BigDecimal.ROUND_HALF_UP);
    }
    ...
    public static boolean equalBigDeci(String one,String two){
        BigDecimal bone = null;
        BigDecimal btwo = null;
        if(Strings.isNullOrEmpty(one)){
            bone = new BigDecimal(0);
        }else{
            bone = new BigDecimal(one);
        }
        if(Strings.isNullOrEmpty(two)){
            btwo = new BigDecimal(0);
        }else{
            btwo = new BigDecimal(two);
        }
        if(bone.compareTo(btwo)==0){
            return true;
        }
        return false;
    }
    public static boolean equalInt(String one,String two){
        int bone =0;
        int btwo =0;
        if(!Strings.isNullOrEmpty(one)){
            bone = Integer.parseInt(one);
        }
        if(!Strings.isNullOrEmpty(two)){
            btwo = Integer.parseInt(two);
        }
        if(bone==btwo){
            return true;
        }
